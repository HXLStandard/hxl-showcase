...
class ReportController extends AbstractController {
  ...
  function doGET(HttpRequest $request, HttpResponse $response) {
    ...
    $value_query = sprintf('select V.* from value_view V join latest_import_view LI on V.import=LI.id where V.code_code in (%s) order by V.row, V.col', implode(',', array_fill(0, count($col_list), '?')));
    $params = $col_list;
